...
class BookingController extends StateNotifier<BookingStates> {
  ...
  final Ref _ref;
  BookingController(this._service, this._ref) : super(BookingInitialState());
...
/*     final tableController = _ref.read(clubTableProvider);
  ...
  bookEvent(List<GuestModel> guests, EventModel event) async {
    try {
      state = BookingLoading();
      // final response = await _service.submitEventBooking(
      //     eventData!.id, guests, eventData!.name, eventData!.entryPerHead);
      final response = await _service.submitEventBooking(
          event.id, guests, event.name, event.entryPerHead);
